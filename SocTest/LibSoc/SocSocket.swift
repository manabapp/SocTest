...
//        let cmsgs: [SocCmsg] = []
    ...
    static func poll(sockets: inout [SocSocket]) -> Int32 {
        var fds: [pollfd] = []
        for i in 0 ..< sockets.count {
            if sockets[i].isClosed {
                continue
            }
            fds.append(pollfd(fd: sockets[i].fd, events: Int16(POLLIN|POLLPRI|POLLOUT), revents: 0))
        }
        if fds.count == 0 {
            return -1
        }
        let startDate = Date()
        let ret = Darwin.poll(&fds, nfds_t(fds.count), 0)
        SocLogger.setResponse(startDate)
        var logFDS = ""
        ...
        SocLogger.trace(funcName: "poll", argsText: "[\(logFDS)], \(fds.count), 0", retval: ret)
        return ret
    }
