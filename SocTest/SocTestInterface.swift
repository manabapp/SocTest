...
struct SocTestInterface {
    ...
    var name: String { return SocTestInterface.deviceNames[self.deviceType] }
    ...
    mutating func ifconfig(isLaunching: Bool = false) {
        ...
        while ifaPtr != nil {
            guard let ifa = ifaPtr?.pointee else {
                SocLogger.error("SocTestInterface.ifconfig: null pointer")
                return
            }
            guard let ifaName = String(validatingUTF8: ifa.ifa_name) else {
                SocLogger.error("SocTestInterface.ifconfig: no name")
                return
            }
        ...
        if self.isActive {
            if self.deviceType == SocTestInterface.deviceTypeLoopback {
